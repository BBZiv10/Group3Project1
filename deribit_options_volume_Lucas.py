...
last_week = dt.datetime.now() - dt.timedelta(days = 8)
...
last_week_beginning = dt.datetime(last_week.year, last_week.month, last_week.day,0,0,0,0)
last_week_beginning_time = (int(time.mktime(last_week_beginning.timetuple()))*1000)
last_week_end = dt.datetime(last_week.year, last_week.month, last_week.day,23,59,59,999)
last_week_end_time = (int(time.mktime(last_week_end.timetuple()))*1000)
...
master_df.to_csv(r'C:\Users\lucas\Desktop\Fintech_Project_Group3\Group3Project1\Resources\Deribit_options_trades.csv', index = False)
